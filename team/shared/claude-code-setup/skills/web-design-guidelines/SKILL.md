...
description: "Semantic HTML, WCAG 2.2, color contrast, keyboard nav, responsive CSS, Core Web Vitals, dark mode, i18n, print"
...
Web implementation issue → Interactive element? → Yes: native HTML? No→SS1.1+SS1.2 / Yes→keyboard(SS1.3)→focus(SS1.4)→name(SS1.2)→contrast(SS1.7) / No: Image→alt(SS1.6), Table→caption+th(SS1.11), Text→contrast+line-height(SS4) → Layout/responsive? → scroll@320px(SS2.7), touch<24px→44px(SS2.5), viewport(SS2.6) → Performance? → Images(SS5.1), Fonts(SS5.2), JS(SS5.3) → Animation/theme? → reduced-motion(SS6) → i18n(SS10) + print(SS11)

---

## Accessibility Quick Review

> Merged from `fixing-accessibility`. Use `/fixing-accessibility <file>` to audit a file against these rules.
...
- Modals: set **initial focus** inside dialog; **restore focus** to trigger on close; prevent background scroll
- Expandable controls: always pair `aria-expanded` with `aria-controls`
- Loading states: use `aria-busy` or `role="status"` text
- Disabled submit: must **explain why** it is disabled (see also SS3.7)
- Tool boundaries: prefer minimal fixes — do not refactor unrelated code or add ARIA when native semantics suffice

### Review Guidance

1. Fix **critical** issues first (names → keyboard → focus → tool boundaries)
2. Prefer native HTML before adding ARIA
3. Quote the exact snippet, state the failure, propose a **small** targeted fix
4. For complex widgets (menu, dialog, combobox), prefer established accessible primitives
...
## 12. 2025-2026 Web Platform 新API [MEDIUM]

- **Popover API**: `popover` 属性 + `popovertarget` で JS 不要のポップオーバー。全主要ブラウザ対応済み
- **View Transitions API**: `document.startViewTransition()` でページ遷移アニメーション（MPA は `@view-transition` CSS）
- **CSS @scope**: `@scope (.card) { ... }` でスコープ付きスタイル（コンポーネント内に限定）
- **CSS Anchor Positioning**: `anchor-name` + `position-anchor` でツールチップ・ドロップダウンの位置決め標準化（Chrome 125+、他ブラウザは順次対応）
...
- [ ] Every form input has a programmatic label (not placeholder-only)
- [ ] No horizontal scroll at 320px viewport width
- [ ] Touch targets are at least 24x24px (AA) / 44x44px (AAA)
- [ ] `prefers-reduced-motion` is respected for all animations
- [ ] Dark mode maintains proper contrast ratios
- [ ] Heading hierarchy is sequential (h1-h6, no skipped levels)
- [ ] Skip navigation link is present for repeated content

---

## Cross-references [MEDIUM]

- `ux-psychology` — 認知心理学に基づく UX の「なぜ」（Fitts's Law、Hick's Law、認知負荷、フォーム心理学）
- `mobile-first-responsive` — LIFF/PWA/Tailwind でのモバイルファースト実装（safe-area、viewport units、タッチ 48dp）
- `vercel-react-best-practices` — React/Next.js ランタイムパフォーマンス最適化（Core Web Vitals、バンドルサイズ、re-render）
