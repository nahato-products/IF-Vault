...
description: "Apply secure-by-default coding patterns for Python, JavaScript/TypeScript, and Go covering input validation, parameterized queries, CSRF protection, secret management, authentication hardening, and OWASP Top 10 prevention. Use when implementing secure coding patterns, hardening authentication flows, adding input sanitization, configuring CORS/CSP headers, managing secrets securely, or requesting best-practice security guidance. Do not trigger for vulnerability detection (use security-review), threat modeling (use security-threat-model), or penetration testing (use security-arsenal)."
...
- **_security-review**: コードベースの脆弱性検出・OWASP Top 10 パターンスキャン
- **_security-threat-model**: リポジトリベースの脅威モデリング・攻撃パス列挙
- **security-arsenal**: Red/Blue チームによる攻撃的・防御的セキュリティ運用
- **typescript-best-practices**: 型安全性による入力バリデーション強化
