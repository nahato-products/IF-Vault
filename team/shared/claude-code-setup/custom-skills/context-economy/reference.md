...
| JSON / YAML（構造化データ） | 約3文字 ≈ 1トークン | ブラケット・コロン等の記号が多い |

**注意:** 画像は1枚で1,000トークン以上消費可能性。本当に必要な場合のみ使用。
...
```
Step 1: Glob: pattern="src/**/*.{ts,tsx}" でファイル一覧
Step 2: Read: limit=30 でエントリポイント先頭確認
Step 3: Grep: pattern="export", head_limit=15, output_mode="content"
Step 4: Read: offset=45, limit=35 で必要箇所のみ
```

### Session Mode（セッション管理）

長時間作業でコンテキスト温存。

```
Phase 1: 通常作業（序盤）
   → offset/limit 付き Read、head_limit 付き Grep を常用
   → 不要な全文 Read 回避
...
Phase 3: 継続 or リセット（終盤）
   → status.md を読んで作業再開
   → キーファイル再読み込みは limit 付き最小限
```

### Audit Mode（診断）

現在セッションのトークン効率振り返り改善。

```
Step 1: セッション中の Read 操作振り返り
   → 500行以上のファイルを全文Readしていないか確認
   → 確認方法: map_dir_cost でトークン数チェック

Step 2: 重複 Read 確認
   → 同一ファイルへの Read が2回以上ないか
   → 重複あれば Knowledge Externalization 適用
...
| 全文が確実に必要な設定ファイル（tsconfig.json 等） | skeleton mode でも結局 force_full 必要 |
| 1回しか読まないファイル | 最適化の判断コスト自体が無駄 |
| デバッグ中で文脈全体が必要 | 部分読みでバグ原因見落とし可能性 |
| ファイル内容が不明で構造把握が最優先 | まず read_smart でskeleton、必要なら追加読み |

**判断基準:** 「Read 1回 + 数秒の判断」と「offset/limit の試行錯誤で2〜3回 Read」を比較、前者が安いなら素直に読む。
