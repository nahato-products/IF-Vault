...
```css
/* iOS Safariで入力フォーカス時のズームを防ぐ */
/* 16px未満のフォントサイズだと自動ズームが発生する */
input, select, textarea {
  font-size: 16px; /* or larger */
}

/* Tailwind: text-base = 16px (safe) */
/* Tailwind: text-sm = 14px (will trigger zoom on iOS!) */
```
...
## 11. Debugging LIFF Apps

### Development Tools

| Tool | Purpose | Setup |
|------|---------|-------|
| `eruda` | Mobile console/inspector | `import('eruda').then(e => e.default.init())` |
| `vconsole` | Alternative mobile console | `import('vconsole').then(V => new V.default())` |
| Chrome Remote Debug | USB debugging (Android) | `chrome://inspect` |
| Safari Web Inspector | USB debugging (iOS) | Safari -> Develop menu |
| ngrok / Cloudflare Tunnel | HTTPS tunnel for LIFF | Required for LIFF endpoint |

### LIFF Testing Checklist

```
Environments to test:
[ ] LIFF browser (iOS LINE app)
[ ] LIFF browser (Android LINE app)
[ ] External browser (iOS Safari)
[ ] External browser (Android Chrome)
...
[ ] Android mid-range (various notch/punch-hole)
[ ] Android budget (slower CPU, less RAM)

Scenarios:
[ ] Keyboard open (all form fields)
[ ] Landscape orientation (if supported)
[ ] Slow 3G network
[ ] Offline / service worker fallback
[ ] Deep link into LIFF
[ ] Close and reopen (cache behavior)
```
...
export default function RootLayout({ children }: { children: React.ReactNode }) {
  return (
    <html lang="ja" className="antialiased">
      <body className="
        flex min-h-svh flex-col bg-white text-gray-900
        overscroll-none
      " style={{
        ...
        {children}
        {/* Bottom padding handled by BottomTabBar component */}
      </body>
    </html>
  );
}
...
export function LiffHeader({ title }: { title: string }) {
  ...
  return (
    <header className="sticky top-0 z-50 flex h-12 items-center justify-between bg-white px-4 shadow-sm">
      <button onClick={handleBack} className="flex size-10 items-center justify-center" aria-label="Go back">
        <ChevronLeft className="size-5" />
      </button>
      <h1 className="text-base font-semibold">{title}</h1>
      {isInClient ? (
        <button onClick={handleClose} className="flex size-10 items-center justify-center" aria-label="Close">
          <X className="size-5" />
        </button>
      ) : (
        ...
        {tabs.map(({ href, icon: Icon, label }) => {
          ...
          return (
            <li key={href}>
              <Link
                href={href}
                className={`flex min-h-12 min-w-12 flex-col items-center justify-center gap-0.5 rounded-lg px-3 py-1 transition-colors ${
...
// メインレイアウトで: <main className="pb-20">...</main>
```
  ...
  return (
    <div className="relative">
      <input
        ...
        placeholder=" "
        className="
          peer w-full rounded-lg border border-gray-300 px-4 pb-2 pt-5
          text-base focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500
        "
        {...props}
      />
      <label className="
