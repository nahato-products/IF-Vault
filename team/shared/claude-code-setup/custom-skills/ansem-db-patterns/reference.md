...
```sql
-- 安全な変更（TEXT→TEXT: PostgreSQLでは意味なし）
-- 危険な変更: 型変換はREWRITEが走る。大テーブルではロック時間に注意
ALTER TABLE m_influencers ALTER COLUMN status_id TYPE INTEGER;
-- ↑ SMALLINT→INTEGERは安全だがINTEGER→SMALLINTはデータ次第で失敗
```

### VARCHAR→TEXT移行

```sql
-- VARCHARからTEXTへの変更はメタデータのみ。テーブルREWRITE不要で即完了
ALTER TABLE m_influencers ALTER COLUMN influencer_name TYPE TEXT;
-- 長さチェックが必要ならCHECK制約で明示的に
...
- [ ] ALTER TABLE はロックが必要か確認（カラム追加はほぼ無ロック）
...
DO UPDATE SET
  ...
  partner_name = EXCLUDED.partner_name,  -- スナップショット更新
  site_name = EXCLUDED.site_name,
  updated_at = CURRENT_TIMESTAMP;
```

### バルクUPSERT（ansem-import用）
...
VALUES
  ($1, $2, $3, $4, $4),
  ($5, $6, $7, $8, $8)
ON CONFLICT (influencer_id) DO NOTHING;  -- 既存はスキップ
```

### 条件付きUPSERT

```sql
-- 新しいデータのみ上書き（updated_atが新しい場合のみ）
...
- Supabase Postgres Best Practices（Skills連携: supabase-postgres-best-practices）
- Neon Serverless Postgres（Skills連携: neon-postgres）
