...
### 提案2: _cache の自動更新トリガー

月替わりでシート名が変わると、_cache の XLOOKUP 参照先も更新が必要。
...
- 参照先が変わった場合（例: XLOOKUP → _cache参照）にまとめシート側にも影響がないか
...
- Step 2 の _cache 作成には XLOOKUP の展開に数秒かかる（`Utilities.sleep(5000)` で待機）
