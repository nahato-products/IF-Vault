...
| ファイル | 行数 | 内容 |
|----------|------|------|
| 001_create_tables | ~1000 | 32テーブルとFK制約 |
| 002_create_indexes | 298 | 複合/部分/GINインデックス |
| 003_create_comments | 473 | テーブルと主要カラムのコメント |
| 004_create_triggers | 75 | updated_at自動更新（関数1＋トリガー30） |
| 005_create_partitions | 78 | 年次/月次パーティションとアーカイブ関数 |
...
投入方法は個別登録（管理画面フォーム）と一括登録（CLIツール ansem-import）の2つ。一括登録用スプシテンプレートは25列で、基本情報・SNS情報・銀行口座・請求先と住所をカバーする。

→ CLIツールとYAML定義の詳細は [[ANSEM-Phase1実装計画]] を参照
...
一括登録（ansem-import）は初回移行だけでなく、月次の新規IF追加や案件開始時にも継続的に使うCLIツール。YAML設定で全テーブルに拡張可能な汎用設計。

→ 詳細は [[ANSEM-Phase1実装計画]] を参照

## 追加機能ロードマップ

14の追加機能を4つのPhaseに分けている。

Phase 1はBQ自動取り込み・一括登録CLI・確認画面・検索機能でデータを入れて確認できる状態を作る。

→ Phase 1の詳細は [[ANSEM-Phase1実装計画]] を参照

Phase 2で請求確定フローに入る。期間選択からフィルタ設定、プレビュー、確定、スナップショット保存まで一気通貫。CSV/Excelエクスポートも実装する。

Phase 3以降はダッシュボード、単価管理UI、担当者アサイン管理、通知機能で運用を改善し、Phase 4でRBAC、監査ログ閲覧、ファイル管理UI、多言語対応に広げる。

## 設計レビューの経緯

v1.0から5回のメジャーバージョンアップを重ねた。レビュー指摘は累計23件、全て対応済み。

v1.0はプレフィックス不統一やER図リレーション誤りなど基本的な問題が多かった。v2.0〜v3.0で命名規約を統一し、ソフトデリート方針とパスワード管理を整理。v4.0では楽観ロック、自動更新トリガー、ON DELETE使い分けルールの明文化が完了した。

v5.0が大きな転換点になった。スケーリング対応でパーティションとPgBouncerを導入し、多言語テーブルとファイル管理も追加。v5.1で細かい品質改善を重ね、品質スコアは8.5から9.2に上がっている。
...
→ ドキュメント全体の関係図は [[ANSEM-設計ナビゲーションマップ]] を参照
